// OnBalanceVolume: same cumulative semantics as BalanceVolume but first
// bar emits 0 (not null).
...
describe('calcOnBalanceVolume', () => {
    it('empty candles → empty', () => {
        assert.deepStrictEqual(calcOnBalanceVolume([], {}), []);
    });

    it('single candle → 0 (not null — diverges from BalanceVolume here)', () => {
        const r = calcOnBalanceVolume([{ time: 't0', open: 1, high: 1, low: 1, close: 10, volume: 100 }], {});
        assert.strictEqual(r.length, 1);
        assert.strictEqual(r[0].value, 0);
    });

    it('hand-computed five-bar series with up / down / flat transitions', () => {
        // bar 0 c=10 v=100 → 0     (seed; .cs short-circuits at prevClose==0)
        ...
        const r = calcOnBalanceVolume(candles, {});
        assert.strictEqual(r[0].value, 0);
    ...
    it('NaN volume bar → null and cumulative does not advance', () => {
        const candles = [
            { time: 't0', open: 10, high: 10, low: 10, close: 10, volume: 100 },
            { time: 't1', open: 11, high: 11, low: 11, close: 11, volume: 100 }, // +100 → 100
            { time: 't2', open: 12, high: 12, low: 12, close: 12, volume: NaN }, // bad → null
            { time: 't3', open: 13, high: 13, low: 13, close: 13, volume: 50 },  // +50 vs prevClose=11 (skipped) → 150
        ];
        const r = calcOnBalanceVolume(candles, {});
        assert.strictEqual(r[0].value, 0);
